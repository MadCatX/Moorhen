...
import type * as LT from "../../types/llka.d.ts";

/*
 * ===
 *
 * Boiler-plate to for WASM initialization
 * and handling
 *
 *                vvv
 * ===
...
/*
 * ===
 * NtC classification is (un)fortunaterly quite parametrizable so
 * a context with the set of classification parameters needs to
 * be set up before we can classify anything
 * ===
 */
let LLKAClassificationCtx: LT.LLKAClassificationContext;
async function initializeLLKAClassificationContext() {
 ...
 * WebWorker logic
 *
 * Most of this is stolen from CootInstance.ts with some very simple
 * libLLKA handling bits put on top of it.
 *
 * ===
 */

/*
 * Types of data exchanged between the Worker and the main application
 */
...
onmessage = function(e) {
    ...
    if (message.command === 'Initialize') {
        ...
        try {
            const result = doSuperposeClosestNtC(message.firstResidue, message.secondResidue);
        ...
        try {
            const result = doSuperposeSpecificNtC(message.firstResidue, message.secondResidue, message.NtC);
...
function doSuperposeClosestNtC(firstResidue: AtomDescription[], secondResidue: AtomDescription[]) {
...
function doSuperposeSpecificNtC(firstResidue: AtomDescription[], secondResidue: AtomDescription[], NtCNumber: number) {
    const stru = atomsToLlkaStructure([...firstResidue, ...secondResidue]);
    const targetFirstBase = stru.get(0).label_comp_id;
    const targetSecondBase = stru.get(stru.size() - 1).label_comp_id;

    try {
        const NtC = NtCNumberToNtCEnum(NtCNumber);
...
/*
 * ===
 * Utility functions that call libLLKA and libcoot
 * to create the piece of structure that will replace
 * its analog that is in the structure now
 * ===
 */
 ...
 * Helper function
 *
 * Needed only for the base replacement
 * ===
 */
...
/*
 * ===
 * We need to get the structure from the libcoot molecule container
 * out in a processable format.
 * So far it seems like exporting it into a file is the only way
 * how to do it
 * ===
 */
...
function fixUpNtCBases(NtC: LT.NtC, targetFirstBase: string, targetSecondBase: string) {
    ...
    // Use liboot to replace the bases
...
/*
 * ===
 * Helper functions to create libLLKA internal representations
 * of structrures
 *
 * vvv
 * ===
...
function CLLKAStructure(): LT.LLKAStructure {
    return LLKAInstance.makeStdVectorStructure();
}

function CLLKAPoint(x: number, y: number, z: number) {
    return { x: x, y: y, z: z };
}
...
function atomsToLlkaStructure(atoms: AtomDescription[]) {
    ...
    for (const atom of atoms) {
        ...
        const a = CLLKAAtom(
            atom.element_name.trim(),
            atom.label_atom_id.trim(),
            '', // Not necessary for NtC classification
            atom.label_comp_id,
            atom.label_asym_id,
            atom.label_atom_id.trim(), // Auth
 ...
 * ===
 *
 * Awkward conversion functions between libLLKA atom representation
 * and PDB. At this point, converting a structure to PDB seems
 * to be necessary to exchange it with libcoot
 * ===
 */

function coortFlt(f: number) {
    return f.toFixed(3).padStart(8, ' ');
}

function llkaAtomToPdbLine(atom: LT.LLKAAtom, serial: number) {
...
/*
 * ===
 * NtC classes are represented and an enum in libLLKA
 * and emscripten exports it as such so we need
...
function NtCToName(ntc: LT.NtC): string {
    return LLKAInstance.NtCToName(ntc);
}

// !!!
// This function uses libcoot
// !!!
function replaceBase(molTainer: libcootApi.MoleculesContainerJS, anchorAtom: LT.LLKAAtom, newBase: string) {
...
function superposeNtC(NtC: LT.NtC, stru: LT.LLKAStructure, targetFirstBase: string, targetSecondBase: string) {
    //
    // The template NtC structures are taken from real molecules and might have
    // different bases than the residues we are replacing. Luckily,
    // libcoot comes to the rescue and can fix up the bases for us
    //
